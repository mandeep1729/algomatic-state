"""Entry point for the contrarian trading agent.
...
setup_logging(
    level=os.environ.get("CONTRARIAN_LOG_LEVEL", settings.logging.level),
    format="text",
    file=LOGS_DIR / "contrarian.log",
...
def _run_api(config: ContrarianAgentConfig) -> None:
...
def main() -> None:
    agent_config = ContrarianAgentConfig()
    strategy_config = ContrarianStrategyConfig()

    set_config(agent_config)

    logger.info(
        "Starting contrarian agent",
        extra={
