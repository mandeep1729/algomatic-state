...
| 1.2 | Define `state_vector_feature_spec.yaml` schema | YAML schema for feature names, formulas, window sizes, scaling policy | [ ] |
| 1.3 | Create `metadata.json` schema | Schema for training window, K, d, universe, version info | [ ] |
...
| 3.4 | (Optional) Denoising autoencoder | Temporal DAE with noise injection for stability | [ ] |
| 3.5 | Encoder serialization | Save to `.pkl` or `.onnx` format | [ ] |
| 3.6 | Latent dimension selection | Explained variance analysis for PCA; reconstruction error for AE | [ ] |
...
| 6.5 | OOD detection | log p(z_t) threshold -> UNKNOWN state | [ ] |
| 6.6 | Multi-timeframe synchronizer | Carry-forward higher TF states until bar close | [ ] |
...
| 9.5 | Retraining scheduler | Configurable cadence per timeframe | [ ] |
